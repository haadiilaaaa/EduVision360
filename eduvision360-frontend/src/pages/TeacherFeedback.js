...
function TeacherFeedback() {
  ...
  const loadFeedback = async () => {
    try {
      const res = await getTeacherFeedback();
  ...
  const handleUpdate = async (id, status, responseComment) => {
    try {
      await updateTeacherFeedbackStatus(id, { status, responseComment });
  ...
  return (
    <div>
      <h2 style={{ color: "var(--accent)" }}>Teacher Feedback Review</h2>
      {message && <p>{message}</p>}
      ...
      {feedbackList.length === 0 ? (
        ...
        feedbackList.map((item) => (
          <TeacherFeedbackCard key={item.id} item={item} onUpdate={handleUpdate} />
        ))
      )}
    </div>
  );
...
function TeacherFeedbackCard({ item, onUpdate }) {
